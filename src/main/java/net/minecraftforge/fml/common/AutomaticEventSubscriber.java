...
public class AutomaticEventSubscriber
{
    ...
    {
        PFServer.LOGGER.debug("Attempting to inject @EventBusSubscriber classes into the eventbus for {}", mod.getModId());
        ...
        for (ASMDataTable.ASMData targ : targets)
        {
            try
            {
                ...
                if (sides == DEFAULT || sides.contains(side)) {
                    //PFServer.LOGGER.debug("Found @EventBusSubscriber class {}", targ.getClassName());
                    ...
                    if (Strings.isNullOrEmpty(amodid)) {
                        ...
                        if (Strings.isNullOrEmpty(amodid)) {
                            PFServer.bigWarning("Could not determine owning mod for @EventBusSubscriber on {} for mod {}", targ.getClassName(), mod.getModId());
                            continue;
                        }
                    }
                    if (!mod.getModId().equals(amodid))
                    {
                        PFServer.LOGGER.debug("Skipping @EventBusSubscriber injection for {} since it is not for mod {}", targ.getClassName(), mod.getModId());
                        continue; //We're not injecting this guy
                    }
                    PFServer.LOGGER.debug("Registering @EventBusSubscriber for {} for mod {}", targ.getClassName(), mod.getModId());
                    Class<?> subscriptionTarget = Class.forName(targ.getClassName(), false, mcl);
                    MinecraftForge.EVENT_BUS.register(subscriptionTarget);
                    PFServer.LOGGER.debug("Injected @EventBusSubscriber class {}", targ.getClassName());
                }
            }
            catch (Throwable e)
            {
                PFServer.LOGGER.error("An error occurred trying to load an EventBusSubscriber {} for modid {}", targ.getClassName(), mod.getModId(), e);
