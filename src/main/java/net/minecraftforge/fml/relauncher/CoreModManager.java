...
public class CoreModManager {
    ...
    private static String[] rootPlugins = { "net.minecraftforge.fml.relauncher.FMLCorePlugin", "net.minecraftforge.classloading.FMLForgePlugin", "mgazul.PFServer.CorePlugin" };
    ...
    private static class FMLPluginWrapper implements ITweaker {
        ...
        {
            PFServer.LOGGER.debug("Injecting coremod {} \\{{}\\} class transformers", name, coreModInstance.getClass().getName());
            List<String> ts = Lists.newArrayList();
            if (coreModInstance.getASMTransformerClass() != null) for (String transformer : coreModInstance.getASMTransformerClass())
            {
                PFServer.LOGGER.trace("Registering transformer {}", transformer);
            ...
            PFServer.LOGGER.debug("Running coremod plugin {}", name);
            ...
            PFServer.LOGGER.debug("Coremod plugin class {} run successfully", coreModInstance.getClass().getSimpleName());
    ...
    public static void handleLaunch(File mcDir, LaunchClassLoader classLoader, FMLTweaker tweaker)
    {
        ...
        try
        {
            ...
            if (bs != null)
            {
                PFServer.LOGGER.info("Managed to load a deobfuscated Minecraft name- we are in a deobfuscated environment. Skipping runtime deobfuscation");
        ...
        {
            PFServer.LOGGER.debug("Enabling runtime deobfuscation");
        }
        ...
        PFServer.LOGGER.debug("All fundamental core mods are successfully located");
        ...
        for (String coreModClassName : commandLineCoremods.split(","))
        {
            ...
            PFServer.LOGGER.info("Found a command line coremod : {}", coreModClassName);
    ...
    private static void findDerpMods(LaunchClassLoader classLoader, File modDir, File modDirVer)
    {
        ...
        {
            PFServer.LOGGER.fatal("FML has detected several badly downloaded jar files,  which have been named as zip files. You probably need to download them again, or they may not work properly");
            for (File f : derplist)
                PFServer.LOGGER.fatal("Problem file : {}", f.getName());
        }
        ...
        {
            PFServer.LOGGER.fatal("There appear to be jars extracted into the mods directory. This is VERY BAD and will almost NEVER WORK WELL");
            PFServer.LOGGER.fatal("You should place original jars only in the mods directory. NEVER extract them to the mods directory.");
            PFServer.LOGGER.fatal("The directories below appear to be extracted jar files. Fix this before you continue.");

            for (File f : derpdirlist)
            {
                PFServer.LOGGER.fatal("Directory {} contains {}", f.getName(), Arrays.asList(new File(f,"META-INF").list()));
            }
            ...
            try
            {
                ...
                PFServer.LOGGER.fatal("#@!@# FML has crashed the game deliberately. Crash report saved to: #@!@# {}", crashreportfile.getAbsolutePath());
            } catch (Exception e)
            {
                PFServer.LOGGER.fatal("#@!@# FML has crashed while generating a crash report, please report this. #@!@#", e);
                // NOOP - hopefully
            }
            throw re;
        }
    ...
    private static void discoverCoreMods(File mcDir, LaunchClassLoader classLoader)
    {
        ...
        PFServer.LOGGER.debug("Discovering coremods");
        ...
        {
            PFServer.LOGGER.debug("Examining for coremod candidacy {}", coreMod.getName());
            JarFile jar = null;
            ...
            try
            {
                ...
                {
                    PFServer.LOGGER.debug("Mod {} has ModSide meta-inf value {}, and we're {} It will be ignored", coreMod.getName(), modSide, FMLLaunchHandler.side.name());
                ...
                {
                    PFServer.LOGGER.info("Loading tweaker {} from {}", cascadedTweaker, coreMod.getName());
                ...
                {
                    PFServer.LOGGER.debug("Adding {} to the list of things to skip. It is not an FML mod, it has types {}", coreMod.getName(), modTypes);
                    ignoredModFiles.add(coreMod.getName());
                    continue;
                }
                ...
                {
                    // Not a coremod
                    PFServer.LOGGER.debug("Not found coremod data in {}", coreMod.getName());
                    continue;
                }
            }
            catch (IOException ioe)
            {
                PFServer.LOGGER.error("Unable to read the jar file {} - ignoring", coreMod.getName(), ioe);
                continue;
            }
            finally
            {
            ...
            try
            {
                ...
                {
                    PFServer.LOGGER.trace("Adding {} to the list of known coremods, it will not be examined again", coreMod.getName());
                    ignoredModFiles.add(coreMod.getName());
                }
                else
                {
                    PFServer.LOGGER.warn("Found FMLCorePluginContainsFMLMod marker in {}. This is not recommended, @Mods should be in a separate jar from the coremod.",
                            coreMod.getName());
            ...
            {
                PFServer.LOGGER.error("Unable to convert file into a URL. weird", e);
                continue;
            }
    ...
    private static void handleCascadingTweak(File coreMod, JarFile jar, String cascadedTweaker, LaunchClassLoader classLoader, Integer sortingOrder)
    {
        ...
        {
            PFServer.LOGGER.info("There was a problem trying to load the mod dir tweaker {}", coreMod.getAbsolutePath(), e);
        }
    }
    ...
    private static FMLPluginWrapper loadCoreMod(LaunchClassLoader classLoader, String coreModClass, File location)
    {
        ...
        try
        {
            PFServer.LOGGER.debug("Instantiating coremod class {}", coreModName);
            ...
            if (coreModNameAnn != null && !Strings.isNullOrEmpty(coreModNameAnn.value()))
            {
                ...
                PFServer.LOGGER.trace("coremod named {} is loading", coreModName);
            }
            MCVersion requiredMCVersion = coreModClazz.getAnnotation(IFMLLoadingPlugin.MCVersion.class);
            if (!Arrays.asList(rootPlugins).contains(coreModClass) && (requiredMCVersion == null || Strings.isNullOrEmpty(requiredMCVersion.value())))
            {
                PFServer.LOGGER.warn("The coremod {} does not have a MCVersion annotation, it may cause issues with this version of Minecraft",
                        coreModClass);
            }
            else if (requiredMCVersion != null && !FMLInjectionData.mccversion.equals(requiredMCVersion.value()))
            {
                PFServer.LOGGER.error("The coremod {} is requesting minecraft version {} and minecraft is {}. It will be ignored.", coreModClass,
                        requiredMCVersion.value(), FMLInjectionData.mccversion);
                return null;
            }
            else if (requiredMCVersion != null)
            {
                PFServer.LOGGER.debug("The coremod {} requested minecraft version {} and minecraft is {}. It will be loaded.", coreModClass,
                        requiredMCVersion.value(), FMLInjectionData.mccversion);
            }
            ...
            if (certList.isEmpty())
            {
                ...
                {
                    PFServer.LOGGER.info("Ignoring missing certificate for coremod {} ({}), we are in deobf and it's a forge core plugin", coreModName, coreModClass);
                }
                else if (deobfuscatedEnvironment && location == null) // This is probably a mod dev workspace
                {
                    PFServer.LOGGER.info("Ignoring missing certificate for coremod {} ({}), as this is probably a dev workspace", coreModName, coreModClass);
                }
                else // This is a probably a normal minecraft workspace - log at warn
                {
                    PFServer.LOGGER.warn("The coremod {} ({}) is not signed!", coreModName, coreModClass);
                }
            }
            else
            {
                PFServer.LOGGER.debug("Found signing certificates for coremod {} ({})", coreModName, coreModClass);
                for (String cert : certList)
                {
                    PFServer.LOGGER.debug("Found certificate {}", cert);
                }
            }
            ...
            {
                PFServer.LOGGER.debug("Added access transformer class {} to enqueued access transformers", accessTransformerClass);
            ...
            PFServer.LOGGER.debug("Enqueued coremod {}", coreModName);
            return wrap;
        }
        catch (ClassNotFoundException cnfe)
        {
            if (!Lists.newArrayList(rootPlugins).contains(coreModClass))
                PFServer.LOGGER.error("Coremod {}: Unable to class load the plugin {}", coreModClass, cnfe);
            else
                PFServer.LOGGER.debug("Skipping root plugin {}", coreModClass);
        }
        catch (ClassCastException cce)
        {
            PFServer.LOGGER.error("Coremod {}: The plugin {} is not an implementor of IFMLLoadingPlugin", coreModClass, cce);
        }
        catch (InstantiationException ie)
        {
            PFServer.LOGGER.error("Coremod {}: The plugin class {} was not instantiable", coreModClass, ie);
        }
        catch (IllegalAccessException iae)
        {
            PFServer.LOGGER.error("Coremod {}: The plugin class {} was not accessible", coreModClass, iae);
        }
        return null;
    }
